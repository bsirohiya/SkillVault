...
function StoryViewer({viewStory, setViewStory}) {
    ...
    const renderContent = ()=>{
        switch (viewStory.media_type) {
            case "image":
                return (
                    <img src={viewStory.media_url} alt="" className='max-w-full max-h-[90vh] object-contain'/>
                )

            case "video":
            return (
                <video onEnded={()=> setViewStory(null)} src={viewStory.media_url} className='max-w-full max-h-[90vh] object-contain' controls autoPlay/>
            )
  ...
  return (
    <div className='fixed inset-0 h-screen z-[110] flex items-center justify-center' style={{backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color: '#000000'}}>
